...
describe "keys type and values type", ->
	...
	test "init", ->
		m = typed TypedMap(Number, Number), new Map([[1,1], [2,2], [3,3]])
		expect([m...]).toEqual([[1,1], [2,2], [3,3]])
	...
	test "trow an error with a non-TypedMap type", ->
		expect(-> m = typed TypedMap(Number, Number), 1)
		.toThrow("Expected map with keys of type 'Number' and values of type 'Number', got Number 1.")

	test "trow an error with a mismatched TypedMap key type", ->
		expect(-> m = typed TypedMap(Number, Number), new Map([[1,1], [true,2], [3,3]]))
	...
	test "set types are stored in TypedMap instances so they do not overwrite", ->
		m1 = typed TypedMap(Number, Number), new Map([[1,1], [2,2], [3,3]])
