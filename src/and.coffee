...
class And extends CustomType
	constructor: (@types...) ->
		...
		for t, i in @types
			CustomType.warn "AnyType is not needed as '#{@helperName}' argument number #{i}." if isAnyType(t)
			CustomType.warn "Literal #{getTypeName(t)} (#{t}) not needed as '#{@helperName}' argument number
