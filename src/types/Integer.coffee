...
class Integer extends Type
	...
	constructor: (n1, n2) ->
		super() # needed
		throw new InvalidTypeError "Integer arguments must be numbers." \
			unless typeof n1 in ['undefined', 'number'] and typeof n2 in ['undefined', 'number']
		switch arguments.length
			when 0 then # nothing
			when 1 then @max = n1
			when 2 then [@min, @max] = [n1, n2]
			else throw new InvalidTypeError "Integer must have at most two arguments." if arguments.length > 2
	validate: (val) ->
